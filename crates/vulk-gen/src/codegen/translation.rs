...
pub struct Translator<'a> {
    c_type_map: HashMap<&'a str, &'a str>,
}
...
impl<'a> Translator<'a> {
    pub fn new(registry: &'a Registry) -> Self {
        ...
        let c_type_map = {
            ...
            for registry_type in &registry.types {
                let registry::TypeCategory::Ctype { ty } = &registry_type.category else {
                    continue;
                };
                map.insert(registry_type.name.as_str(), ty.as_str());
            }
            map
        };
    ...
    pub fn c_type(&self, c: &str) -> Result<&str> {
        self.c_type_map
            .get(c)
            .copied()
    ...
    pub fn vk_complex_type(
        ...
        // Base name.
        let (mut rs_type, built_in_type) = if let Ok(c_type) = self.c_type(vk_type) {
            ...
            (Self::vk_simple_type(vk_type)?, false)
        };
...
mod tests {
    ...
    fn vk_simple_ident() {
        ...
        for (input, expect) in cases {
            assert_eq!(Translator::vk_simple_ident(input).unwrap(), expect);
        }
    }
    ...
    fn vk_enum_member() {
        ...
        for (input_name, input_member, expect) in cases {
            assert_eq!(
                Translator::vk_enum_member(input_name, input_member).unwrap(),
                expect
            );
        }
    }
    ...
    fn vk_bitmask_member() {
        ...
        for (input_name, input_member, expect) in cases {
            assert_eq!(
                Translator::vk_bitmask_member(input_name, input_member).unwrap(),
                expect
            );
        }
    }
